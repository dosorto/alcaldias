            ...
            <div>
                <form action="{{ route('roleUpdate', $role->id) }}" method="post" class="form-update">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" value="{{ $role->name }}" class="form-input mt-1 block w-full rounded-md border-gray-300 @error('name') border-red-500 @enderror" id="name">
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-textarea mt-1 block w-full rounded-md border-gray-300 @error('description') border-red-500 @enderror" name="description" id="roleDescription" rows="3">{{ $role->description }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>    
                        @enderror
                    </div>
                ...
                <div class="bg-gray-100 p-4 rounded">
                    ...
                    <div class="max-h-60 overflow-auto">
                        ...
                        <div class="flex items-center mb-2">
                            <input type="checkbox" id="permission_{{ $permission->id }}" name="permissions[]" value="{{ $permission->id }}" class="form-checkbox" {{ $role->permissions->contains($permission) ? 'checked' : '' }}>
