...
<nav class="flex" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
      ...
      <li>
        <div class="flex items-center">
          ...
          <a href="/barrios" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Barrios</a>
        </div>
      </li>
  ...
  <div class="columns-2">
    ...
    <div class="max-w-6xl mx-auto bg-white p-6 rounded shadow-md dark:bg-gray-900">
        <h2 class="text-3xl font-extrabold dark:text-white">Barrios</h2>
        ...
        <div
            ...
            <div>
                <div class="inline-flex rounded-md shadow-sm" role="group">
                    <button wire:click="openModalCreateBarrios()" type="button"
                ...
                <tbody>
                    @forelse ($barrios as $barrio)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td>{{ $barrio->latitud }}</td>
                            <td>{{ $barrio->longitud }}</td>
                            <td>{{ $barrio->aldea->nombre }}</td>
                            <td>{{ $barrio->aldea->municipios->name }}</td>
                            <td>{{ $barrio->aldea->municipios->departamentos->name }}</td>
                            <td>{{ $barrio->aldea->municipios->departamentos->paises->nombre }}</td>
                            <td class="px-6 py-4">
                                <button wire:click="editBarrio({{ $barrio->id }})" type="button"
                                ...
                                <button wire:click="remove({{ $barrio->id }})" type="button"
    ...
    @if ($createModal)
        @include('livewire.barrios.create')
    @endif

    @if ($updateModal)
        @include('livewire.barrios.update')
    @endif
